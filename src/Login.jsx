      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 'calc(100vh - 68px)' }}>
        <div style={{ flex: 1, backgroundColor: '#000', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '4rem 5rem', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <p style={{ color: '#555', fontSize: '15px', lineHeight: 1.8, maxWidth: '380px', fontFamily: 'Inter' }}>Sign in to access your personalized dashboard — whether you are a student tracking your journey or a teacher managing your courses.</p>
            <div style={{ marginTop: '3rem', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {[['?','Student','Browse events, view grades, track attendance'],['?','Teacher','Manage courses, upload grades, mark attendance']].map(([icon, role, desc]) => (
                <div key={role} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '14px 18px', border: '1px solid #1a1a1a', backgroundColor: '#0a0a0a' }}>
                  <span style={{ color: '#fff', fontSize: '10px' }}>{icon}</span>
                  <div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ marginBottom: '1.8rem' }}>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <button type='button' onClick={() => setShowPassword(!showPassword)} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#999', fontSize: '11px', fontFamily: 'Space Grotesk', fontWeight: 700, letterSpacing: '0.5px' }}>{showPassword ? 'HIDE' : 'SHOW'}</button>
                </div>
              </div>
